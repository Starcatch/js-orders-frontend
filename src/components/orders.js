class Orders {
  ...
  initBindingsAndEventListeners() {
    ...
    this.ordersContainer.addEventListener(
      "submit",
  ...
  renderSortedOrders(sortedOrders) {
    ...
    this.ordersContainer.innerHTML = sortedOrders
      .map((order) => order.renderOrder())
      .join("");
  }
  ...
  handleFormOnSubmit(e) {
    e.preventDefault();
    const item = {
      item_name: e.target.querySelector("#order-item-item_name").value,
      item_price: e.target.querySelector("#order-item_price").value,
  ...
  render() {
    this.ordersContainer.innerHTML = this.orders
      .map((order) => order.renderOrder())
      .join("");
  }
}
